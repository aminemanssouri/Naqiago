...
export default function SplashAnimation({ onFinish }: Props) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      Animated.sequence([
        ...
        Animated.timing(rippleOpacity, {
          ...
          if (!finished && rippleDone) {
            finished = true;
            // Stop bubble sound when splash animation finishes
            if (bubbleSoundRef) {
              bubbleSoundRef.stopAsync().then(() => {
                bubbleSoundRef.unloadAsync();
              }).catch(() => {
                // Ignore errors
              });
            }
    ...
    return () => {
      finished = true;
      // Cleanup bubble sound if component unmounts
      if (bubbleSoundRef) {
        bubbleSoundRef.stopAsync().then(() => {
          bubbleSoundRef.unloadAsync();
        }).catch(() => {
  ...
  }, [dropAnim, scaleAnim, rotateAnim, rippleOpacity, onFinish]);
