...
export default function BookingDateTimeScreen() {
  ...
  const handleContinue = () => {
    if (!selectedDate || !selectedTime) {
      Alert.alert(t('missing_fields_alert'), t('please_select_date_and_time'));
      return;
    }
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Card style={[styles.sectionCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t('select_date_label')} *</Text>
          </View>
          ...
          <View style={styles.quickDateOptions}>
            {dateOptions.slice(0, 3).map((date) => {
              ...
              const label = date.isToday ? t('today_label') : date.isTomorrow ? t('tomorrow_label') : date.weekday;
              return (
                <Pressable
                  key={date.value}
        ...
        <Card style={[styles.sectionCard, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          ...
          <View style={styles.timePeriodsContainer}>
            {[
              { label: t('morning_period'), slots: timeSlots.slice(0, 8), icon: '🌅' },
              { label: t('afternoon_period'), slots: timeSlots.slice(8, 16), icon: '☀️' },
              { label: t('evening_period'), slots: timeSlots.slice(16), icon: '🌆' }
            ].map((period) => (
        ...
        {selectedDate && selectedTime && (
          <Card style={[styles.summaryCard, { backgroundColor: colors.surface, borderColor: colors.cardBorder }]}>
            <Text style={[styles.summaryTitle, { color: colors.textPrimary }]}>{t('selected_appointment')}</Text>
        ...
        <SafeAreaView style={[styles.modalContainer, { backgroundColor: colors.bg }]}>
          <View style={[styles.modalHeader, { borderBottomColor: colors.cardBorder }]}>
            <Pressable onPress={() => setShowCalendar(false)}>
              <Text style={[styles.modalCancelText, { color: colors.textSecondary }]}>{t('cancel_button')}</Text>
            </Pressable>
            <Text style={[styles.modalTitle, { color: colors.textPrimary }]}>{t('select_date_button')}</Text>
            <Pressable onPress={() => setShowCalendar(false)}>
              <Text style={[styles.modalDoneText, { color: colors.accent }]}>{t('done_button')}</Text>
            </Pressable>
          </View>
