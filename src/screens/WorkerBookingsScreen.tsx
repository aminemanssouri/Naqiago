...
import { View, Text, StyleSheet, ScrollView, Alert, Pressable, Modal, Dimensions, ActivityIndicator, Linking } from 'react-native';
import { Calendar, Clock, MapPin, Phone, CheckCircle, XCircle, AlertCircle, Filter, Play } from 'lucide-react-native';
...
export const WorkerBookingsScreen: React.FC = () => {
  ...
  const handleCallCustomer = async (phone?: string) => {
    ...
    if (!number) {
      Alert.alert(t('unavailable'), t('customer_phone_not_available'));
      return;
    }
    try {
      ...
      try {
        ...
        Alert.alert(t('error'), t('unable_to_open_phone_app'));
      }
    }
  };
  ...
  const handleRejectBooking = async (bookingId: string) => {
    try {
      ...
      Alert.alert(t('notice'), t('booking_rejected'));
      loadWorkerBookings(); // Reload bookings
    } catch (error) {
    ...
    <Card style={[styles.bookingCard, { backgroundColor: theme.card, borderColor: theme.cardBorder }]}>
      ...
      <View style={styles.bookingDetails}>
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={[styles.detailText, { color: theme.textSecondary }]}>{t('contact_customer')}</Text>
          <Pressable
            onPress={() => handleCallCustomer(booking.customerPhone)}
      ...
      {type === 'pending' && (
        <View style={styles.bookingActions}>
          ...
          >
            ...
            <Text style={[styles.actionButtonText, { color: '#ef4444' }]}>{t('reject')}</Text>
          </Button>
          <Button
